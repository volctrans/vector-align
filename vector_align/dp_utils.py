...
from vector_align.dp_core import make_dense_costs, score_path, sparse_dp, make_sparse_costs, \
    dense_dp
    ...
    for ii, overlap in enumerate(range(1, num_overlaps + 1)):
        for jj, out_line in enumerate(layer(lines, overlap)):
            try:
                ...
                logger.warning('Failed to find overlap=%d line "%s". Will use random vector.',
                               overlap, out_line)
                line_id = None
...
def make_del_knob(e_laser,
    ...
    if e_size > 0 and f_size > 0 and sample_size > 0:
        ...
        if e_size * f_size < sample_size:
            ...
            # get random samples
            x_idxs = np.random.choice(range(e_size), size=sample_size, replace=True).astype(
                np.int32)
            y_idxs = np.random.choice(range(f_size), size=sample_size, replace=True).astype(
                np.int32)
...
def compute_norms(vecs0, vecs1, num_samples, overlaps_to_use=None):
    ...
    if overlaps_to_use is not None:
        if overlaps_to_use > overlaps1:
            raise Exception(
                'Cannot use more overlaps than provided. You may want to re-run make_verlaps.py with a larger -n value')
    else:
    ...
    if size1 and samps_per_overlap:
        ...
        for overlap_ii in range(overlaps_to_use):
            ...
            vecs1_rand_sample[overlap_ii * samps_per_overlap:(overlap_ii + 1) * samps_per_overlap,
            :] = random_vecs
...
def vecalign(vecs0,
             ...
             norms1=None):
    if width_over2 < 3:
        logger.warning('width_over2 was set to %d, which does not make sense. increasing to 3.',
                       width_over2)
        width_over2 = 3
    ...
    for depth in stack:
        ...
        if depth == 0 and norms0 is not None:
            ...
            stack[depth]['n0'] = compute_norms(stack[depth]['v0'], stack[depth]['v1'],
                                               num_samps_for_norm)
        ...
        if depth == 0 and norms1 is not None:
            ...
            stack[depth]['n1'] = compute_norms(stack[depth]['v1'], stack[depth]['v0'],
                                               num_samps_for_norm)
    ...
    for depth in stack:
        ...
        stack[depth]['del_penalty'] = stack[depth]['del_knob'].percentile_frac_to_del_penalty(
            del_percentile_frac)
    ...
    for depth in upsample_depths:
        if max_depth > 0:  # upsample previoius alignment to current resolution
            ...
            extend_alignments(course_alignments, stack[depth]['size0'],
        ...
        t0 = time()
        stack[depth]['a_b_costs'], stack[depth]['b_offset'] = make_sparse_costs(stack[depth]['v0'],
                                                                                stack[depth]['v1'],
                                                                                stack[depth]['n0'],
                                                                                stack[depth]['n1'],
                                                                                stack[depth][
        ...
        tt = time() - t0
        num_dot_products = len(stack[depth]['b_offset']) * len(
            stack[depth]['alignment_types']) * width_over2 * 2
        logger.debug(
            '%d x %d sparse DP (%d alignment types, %d window) make features: %.6fs (%.3e per dot product)',
        ...
        stack[depth]['new_b_offset'] = sparse_dp(stack[depth]['a_b_costs'],
                                                 stack[depth]['b_offset'],
        ...
        stack[depth][akey], stack[depth]['alignment_scores'] = sparse_traceback(
            stack[depth]['a_b_csum'],
            stack[depth]['a_b_xp'],
            stack[depth]['a_b_yp'],
    ...
    for key in runtimes:
        if runtimes[key] > 5e-5:
            logger.info(key + ' took ' + '.' * (max_key_str_len + 5 - len(key)) + (
                    '%.4fs' % runtimes[key]).rjust(7))
